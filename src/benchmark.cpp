...
void wall_time_compare_large_num_mult (size_t M_len, size_t tile_size, size_t align_len, std::vector<double>& malloc_ms, std::vector<double>& lin_alloc_ms) {
    ...
    auto t2 = clk::now();
    std::vector<uint16_t> C_malloc(M_len * M_len);
    ...
    auto t4 = clk::now();
    std::vector<uint16_t> C_lin_alloc_aligned(M_len * M_len);
    uint16_t* C_lin_alloc_aligned_ptr = reinterpret_cast<uint16_t*>(C_lin_alloc_aligned.data());

    for (int i = 0; i < 100000; i++) {
        gemm_linear_alloc_aligned(A_ptr, B_ptr, C_lin_alloc_aligned_ptr, M_len, tile_size, align_len);
    }
...
int main () {
    ...
    */
    ...
    for (int i = 0; i < tile_size_sweep.size(); i++) {
        wall_time_compare_large_num_mult(32, tile_size_sweep[i], 64, gemm_malloc_duration_ms, gemm_linear_alloc_duration_ms);
    }
}